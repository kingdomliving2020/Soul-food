"""Attach Snack Pack deliverables (batch 1) to their synthetic format-aware keys.
Overwrites the same blob if a doc is already attached to the key; else inserts.
Run:  cd /app/backend && python3 -m scripts.attach_snackpack_batch1
"""
import json
import os
import sys
...
JOBS = [
    (f"{BASE}/emxfcv9t_BKFT_AE_SnackPack_Month3.epub", "breakfast-ae-month3-snackpack-epub", "BKFT_AE_SnackPack_Month3.epub", "application/epub+zip", "epub"),
    (f"{BASE}/301bxp4o_BKFT_YE_SnackPack_Month1.epub", "breakfast-ye-month1-snackpack-epub", "BKFT_YE_SnackPack_Month1.epub", "application/epub+zip", "epub"),
    (f"{BASE}/9pjw3tak_BKFT_YE_SnackPack_Month2.epub", "breakfast-ye-month2-snackpack-epub", "BKFT_YE_SnackPack_Month2.epub", "application/epub+zip", "epub"),
    (f"{BASE}/jxz5elu2_BKFT_YE_SnackPack_Month3.epub", "breakfast-ye-month3-snackpack-epub", "BKFT_YE_SnackPack_Month3.epub", "application/epub+zip", "epub"),
    (f"{BASE}/n3jqvtsh_BKFT_AE_SnackPack_Month3.pdf", "breakfast-ae-month3-snackpack-ipdf", "BKFT_AE_SnackPack_Month3.pdf", "application/pdf", "pdf"),
]
    ...
    for url, key, fname, ct, ext in JOBS:
        ...
        if doc:
            ...
            db.files.update_one({"_id": doc["_id"]}, {"$set": {
                ...
                "description": f"Snack Pack {role.upper()} for {key}"}})
            ...
            db.files.insert_one({
                ...
                "original_filename": fname, "content_type": ct, "size_bytes": size,
                "etag": etag, "description": f"Snack Pack {role.upper()} for {key}",
                "is_deleted": False, "uploaded_by_admin": "snackpack-batch-2026-07",
                ...
                "attachments": [{"id": str(uuid.uuid4()), "target_type": "product",
                                 ...
                                 "attached_by_admin": "snackpack-batch-2026-07"}]})
            action = "inserted"
