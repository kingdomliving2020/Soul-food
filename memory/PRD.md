...
- [x] **#1 Recipient email reliability.** Root cause: buyer + recipient emails were sent sequentially inside a single try/except in a one-shot (`payment_status != paid`) block; a failure in one aborted the other, errors were swallowed, and it never retried → recipient needed a manual resend. Fix (`payment_routes.py`): new `_send_order_emails()` sends buyer and recipient INDEPENDENTLY (isolated try/except), persists `buyer_email_sent`/`recipient_email_sent` flags + errors, stores `email_dl_payload` for retry; added an already-paid branch in `/checkout/status` that re-sends only unsent emails on subsequent polls (natural retry while success page is open). Unit tests `tests/test_email_reliability.py` (4/4). ⚠️ NOT e2e-testable in preview (Stripe key revoked); user validates delivery in production. NOTE: webhook path (`/webhook/stripe`) still uses its own inline email send — left untouched this pass.
- [x] **#2/#7 Game passes are access-only.** Removed `PRODUCT_FILES` mappings `gaming-pass-30/90-*-digital → holiday-*-full.pdf` so a game pass can never deliver a workbook/eBook. Passes grant timed access via `_grant_game_pass_for_items` only. Workbook AE/YE/IE→file mapping audited clean (no cross-edition contamination; prior Youth→Adult bug remains fixed).
- [x] **#4 Audio Companion audit** (report only): `/app/memory/audio_companion_audit_2026-06.md`. Wording uses approved phrases; no "Free/Bonus Audio". GAPS: not surfaced on Quick Order (G1); policy says paperback-only but backend also grants on Holiday digital full workbook (G2 — needs product decision).
- [x] **#7 Catalog organization review** (report only): `/app/memory/catalog_organization_review_2026-06.md`. Same curriculum surfaced 3+ ways (Featured/Instant-Access/product-card/bundles); eBook highlighted as if Holiday-only when it's a cross-curriculum format.
- [x] **Clarity pass (copy/labels only) — verified iteration_57.json (100%, 8/8).** Applied customer-facing FORMAT terminology site-wide (internal SKUs unchanged): **Workbook (Print)** = physical/POD · **Digital Workbook** = fillable (internal `ipdf`) · **eBook** = internal `epub` · **eLesson** = online lesson (internal `interactive`), used ONLY on Nibbles/single-lessons/samples. Added `FORMAT_LABELS`/`formatLabel()` in `QuickOrder.js`. Made `availableFormats` package-aware and **removed the duplicate `interactive` "i-PDF" option from Full Workbooks & Snack Packs** (now Digital Workbook + eBook [+ Workbook print]); Nibbles show eLesson only. Relabeled AE-Pro names ((Digital Workbook)/(eBook)), Featured bundle cart names (eBook + Snack Pack), SnackPacks page (eLesson/online lessons), ProductSelectionModal medium labels (Digital Workbook / Workbook (Print / POD)). Forbidden-token scan on /quick-order: 0 occurrences of i-PDF/iPDF/ePub/Interactive/"PDF Download (Interactive)". Audio Companion wording corrected to "Every 4 C's full workbook (Print or Digital) includes Exclusive Audio Companion Access" (matches fulfillment; approved phrasing).
- [ ] **STILL OPEN (broader clarity — needs user direction, borders 'no redesign'):** the flat `workbooks` product card + Featured/Instant-Access still surface the same 4 C's curriculum multiple ways (e.g. package rows "Holiday Digital (Adult)" / "Holiday Paperback (Adult)"). De-duplicating these into one Curriculum→Format→Extras presentation is the remaining item from `catalog_organization_review_2026-06.md`.
...
- [x] **#2 Small Group / Book Club bundles expose a PHYSICAL lane.** `_item_is_physical` now True for `isSmallGroupBundle`/`isBookClub`/name-blob bundles (they ship participant booklets); `_item_is_digital` returns False for them (no phantom digital lane). Manual review retained. Verified: SGB shows physical lane + review, completes on physical delivered + manual fulfilled.
- [x] **#3 Quantity alone no longer forces manual review.** Removed the `qty>=5` OR-trigger. Manual review now driven ONLY by complexity: mixed-medium · IE curriculum · Small Group/Book Club bundle · hybrid bundle. Verified: 10× digital self-purchase → `complete`/auto (no review); 20× physical → physical lane/no review.
- [x] **#4 Merch/medallions excluded from IE review.** New `_is_merch_or_medallion()` guard in `_is_ie_item` (matches medal/medallion/merch/pen/bookmark/study-kit/mag- in id/sku/name). An `edition=IE` medallion is physical-only, not instructor curriculum. Verified.
- [ ] **#5 (kept as-is per user):** full Break*fast digital workbooks (`breakfast_*_digital`) remain hard-gated in `is_deliverable` (personal-study). Do not change without separate review.
- Regression test: `tests/test_fulfillment_classification.py` (8 cases). QA seed adds a HYBRID order: `scripts/seed_qa_orders.py` (8 orders; re-run to inspect, `--clean` to remove).
- **HOLD (user directive):** automated fulfillment/reminder/gift-opened emails + APScheduler scheduler are PARKED until fulfillment/entitlement accuracy is signed off. Templates already exist in `email_service.py` (unwired except `send_shipping_notification`).



## Recurring bug fixes — edition mismatch + gift download exposure (July 24, 2026) — verified iteration_54.json (6/6)
- [x] **BUG A (Youth→Adult mismatch):** selecting **YE** for the "Full Table Experience" bundle delivered/labelled the **Adult** SKU (`holiday_ae`). Root cause: at checkout, `normalize_product_id('full-table-experience-ye')` collapsed the `-ye` suffix (display-name heuristics) to the edition-less `full-table-experience`, whose BUNDLE_EXPANSIONS entry maps to `['holiday_ae']`. Fix (`payment_routes.py` normalize_product_id ~474): **early-return when the id is already an exact BUNDLE_EXPANSIONS key**, so edition-specific bundle ids are never collapsed. Verified: `-ye` → `['holiday_ye', youth snack pack]`, `-ae` → `['holiday_ae', adult snack pack]`; non-bundle `-ipdf` variants still collapse (no regression). NOTE: fix applies to NEW orders; any already-placed youth order still has adult download_links and would need a manual admin re-grant.
- [x] **BUG B (gift buyer sees downloads):** for a GIFT/third-party purchase the buyer's confirmation showed 'Your Downloads' with the recipient's tokens. Fix (`payment_routes.py` get_download_links ~3435): endpoint now loads the transaction, computes `is_gift = purchase_type=='gift' OR is_gift flag OR recipient!=buyer`, and returns `{links:[], count:0, is_gift:true, recipient_email}` for gifts (self orders unchanged). Frontend `PaymentSuccess.js` renders a 🎁 gift panel (data-testid `gift-confirmation-panel`) instead of downloads when `is_gift`. This fix is RETROACTIVE (checks purchase_type live) — existing gift orders are corrected immediately after redeploy.
- Left as-is (defensive footgun, low-risk): edition-less `full-table-experience` generic key still defaults to adult; the fix intercepts the real `-ye`/`-ae` paths. Regression test: `tests/test_bug_ab_iter54.py`.
...
- **Fix (`AdminConsole.js`):** On mount the console now probes `GET /api/admin/dashboard` to verify AUTHORIZATION (`authState`: checking|ok|denied|error). On 401/403 it renders an "Admin sign-in required" gate (data-testid `admin-access-denied`) with a "Sign in again" button (`admin-relogin-btn`) that clears the token and routes to `/auth` — signing in again refreshes permissions. Network error → retry screen (`admin-auth-error`). Only success renders the console.
- **Also cleaned (own recent lifecycle code, per review):** removed dead `(emailed or True)` in `_digital_lane`; a paid-but-cancelled order no longer mislabels payment axis as "refunded" (shows paid/pending unless an actual refund occurred).
- **Verified — iteration_51.json (frontend 5/5 PASS):** valid admin loads real data; member token → denied gate (not empty console); re-login clears token → /auth; invalid/expired token → denied gate; /admin/orders lifecycle axes still render. Member test account added to test_credentials.md (qa_member@example.com / MemberTest123!).
- **Open (from code review, NOT yet actioned — awaiting user direction):** (1) MEDIUM — checkout builds Stripe line items from client-sent prices (`payment_routes.py` ~2496/2532); should re-derive from server catalog. (2) MEDIUM — partial refunds subtract the FULL order total from net revenue in `/orders/summary` (`admin_routes.py` ~1555). Benign: `/api/payments/products` healthy (200); the console "Failed to fetch" is aborted-fetch-on-navigation noise.


## Order Fulfillment & Management — 3-Axis Status Model (July 23, 2026)
Separates PAYMENT / FULFILLMENT / ORDER status so a paid order is never falsely "complete" while physical, recipient, or manual work is outstanding. Admin-only (customer Order History untouched pre-launch).
- [x] **Backend (`routes/admin_routes.py`) `_compute_lifecycle` rewritten** to emit, alongside the legacy keys: `payment_status` (pending|paid|refunded), and a `fulfillment` object with 4 independent lanes — `digital` (pending→delivered), `physical` (pending→packed→shipped→delivered), `manual` (pending_review→fulfilled), `recipient` (pending→confirmed) — each `{applicable,status}`; plus `order_status` (open|complete|pending_payment|refunded|cancelled|archived), `order_status_label`, `pending_obligations`, `fulfillment_summary`, `requires_manual_review`.
- [x] **Completion rule:** ORDER = `complete` only when EVERY applicable lane is satisfied. Mixed digital+physical stays OPEN after digital delivery until physical delivered AND admin review done. Digital "delivered" = access granted + links generated + email sent (no actual download required).
- [x] **Manual-review triggers** (`_order_requires_manual_review`): Instructor Edition SKU, Small Group/Book Club bundle, any item qty ≥ 5, OR mixed digital+physical. **Recipient auto-confirms** on claim/login/download (`_recipient_lane`), with manual admin override. Robust medium detection added (`_item_is_physical`/`_item_is_digital`) so format/medium/name — not just PRODUCTS flags — classify items.
- [x] **New admin endpoints:** `POST /orders/{n}/manual-fulfillment` {status}, `POST /orders/{n}/recipient-access` {confirmed}. Existing `POST /orders/{n}/fulfillment` (physical packed/shipped/delivered + tracking + notify) reused.
- [x] **Admin UI (`AdminOrders.js`):** each row shows three labeled axes (`data-testid=lifecycle-axes-<order>`): PAYMENT · FULFILLMENT (one badge per applicable lane) · ORDER (open/complete). Expanded row adds a **Recipient Access** panel (gift) and **Admin Fulfillment Review** panel (bulk/church/IE/mixed) with Mark Fulfilled / Reopen / Confirm / Clear controls, alongside existing physical + ownership controls. Buyer vs Recipient shown (purchased-by / granted-to).
- [x] **Verified — iteration_50.json (100%, backend 12/12 + full UI transitions):** 7 seeded QA orders (digital/physical/mixed/gift/IE/SGB/bulk) display correct axes; MIXED stays open through physical delivery until manual review fulfilled → then complete (proves digital alone never closes); physical-only completes on delivered; gift completes on recipient confirm; IE/BULK/SGB complete on manual fulfilled; reopen works. Regression test `tests/test_lifecycle_admin.py` (auto-reseeds). QA orders cleaned up after (re-run `scripts/seed_qa_orders.py` to inspect live).
...
## What's Implemented
### AE-Pro Format SKUs + Cart Format-Preservation Fix (July 22, 2026)
- [x] **Critical bug fixed:** `CartContext.js` was dropping the `format` field on custom cart items, so checkout never sent `format` to the backend — the format-aware fulfillment branch never fired, and epub/ipdf purchases would have delivered the legacy interactive PDF instead of the real ebook/fillable file. Now `CartContext` preserves `format` + `product_id`. Verified: snack pack / full workbook / holiday epubs now resolve to the correct Object Storage epub/ipdf (previously collapsed to PDF).
- [x] **In His Image AE-Pro** split into two SKUs with $1.50 differentiation: `ihi-ae-pro-ipdf` ($11.99) + `ihi-ae-pro-epub` ($10.49). Uses a `deliverKey` on the package → `addToCart` emits clean `product_id` + `format:'digital'` so the resolver hits the direct attachment (avoids the `-physical` suffix pollution that had AE-Pro digital mis-flagged as physical/undeliverable). Files attached to those keys. Verified end-to-end (prices render, cart carries product_id/format, resolver returns correct file).
- [x] Added `-ipdf` base mappings in `payment_routes.py` PRODUCT_FILES mirroring `-epub` (full workbooks, snack packs, holiday full, breakfast IE) so iPDF format resolves to synthetic `<stem>-ipdf` keys instead of normalizing to the wrong product.
- [x] **All 24 digital deliverables attached & verified** (12 ePub + 12 iPDF across BKFT/HOL full workbooks AE/YE/IE + Snack Packs AE/YE M1-3) + AE-Pro ePub/iPDF. Scripts: `scripts/replace_attach_epubs.py`, `attach_snackpack_batch1.py`, `attach_batch2/3/4.py`, `attach_batch5_ihipro.py`. Manifest count 128.
- [x] **iPDF gate lifted (July 22):** Turned off the "iPDF (Coming Soon)" `disabled` gate in the Quick Order format dropdown for Full Workbook + Snack Pack packages. iPDF now selectable and delivers the correct Object Storage file (verified: `holiday-full-adult-ipdf`→`holiday-ae-full-ipdf`, snack packs, breakfast full). Kept **disabled for Nibbles** (labeled "iPDF (n/a for Nibbles)") since Nibbles are Interactive-only per taxonomy and have no iPDF files.
- [x] **Offline Game Master — Breakfast Edition (July 22):** New Quick Order tile (in mealSeries) — single product with **BM1/BM2/BM3 module selector + "All 3 Modules" bundle**, **PDF (Print & Play) / Physical Pack** format selector, pricing $10.00 per module / $25.99 bundle (physical = same base, S&H added separately at checkout), **pre-order** badged, per-module cover art (`/covers/ogm-bkft-*.jpg`). Added `formatLabels` per-product override to the format dropdown. Verified render/pricing/cart. Shipping auto-handled by existing `calculate_shipping` (physical detected via format + `-physical` id; digital exempt).
- ✅ **Game Master digital PDFs ATTACHED & VERIFIED (July 22, 2026):** User supplied final v9 PDFs. Attached to exact delivery keys via `scripts/attach_game_master_bkft.py`: `offline-game-master-bkft-bm1-all-digital`→BM1 (Prayer Is the First Resort, 70pp), `-bm2-`→BM2 (The Art of Through, 76pp), `-bm3-`→BM3 (Faith & Foresight, 70pp), `-bundle-`→MAIN (complete combined 3-module pack, 198pp). Manifest count 128→132.
  - 🐛 **Resolver bug fixed:** `resolve_display_name_to_product_id` treated the `bkft` token as "breakfast" and hijacked bm1/bm2/bm3 cart ids → `breakfast_ae_digital` (wrong PDF); only `-bundle-` survived (via `is_bundle`→None short-circuit). Fix in `resolve_item_to_file_entries_async` (`payment_routes.py` ~928): **exact-attachment on raw cart id now wins BEFORE normalize aliasing** (placed after the epub/ipdf format-aware branch, before the normalized `_has_product_attachment`). Verified all 4 GM keys resolve to their own PDFs + retrievable=True; regression-checked holiday interactive/ipdf/epub, AE-Pro ipdf/epub, snack pack — all still correct.
  - ⚠️ Physical Game Master pack still "ships next week". Storefront tile still shows `[PRE-ORDER]` for BOTH formats — digital now delivers instantly, so a pre-order→instant flip for the DIGITAL format is a pending UX decision. HOL GM pack: user will supply files separately ("i will update HOL GM pk").

## What's Implemented (prior)
...
- [x] **Frontend** (`QuickOrder.js`): added `fillable` to Breakfast (`meal`) and Holiday (`full`) formats + pricing; ePub = Fillable − $1.50 (Breakfast full 14.99→13.49; Holiday full 9.99→8.49; IE 34.99→33.49). Interactive ("i-PDF") price/label unchanged. **Fillable PDF is shown but gated as "Fillable PDF (Coming Soon)" and DISABLED** in both format dropdowns — it cannot be selected/purchased until the real fillable PDFs arrive, so no customer can pay the fillable premium and receive a non-fillable file. Keeps the 3-format taxonomy visible while the deck stays self-contained.
- [x] **Verified (byte-level)**: all 4 ePubs fetch from Object Storage as valid EPUBs (PK-zip signature + `application/epub+zip` mimetype, sizes match: BKFT_AE 10.8MB, BKFT_YE 29.0MB, HOL_AE 11.5MB, HOL_YE 13.5MB). Resolver serves EPUB for ePub format, existing PDF for Interactive — no cross-contamination.
- ⚠️ PREVIEW-only until redeploy. After redeploy, prod autoseeds the 4 ePub records and format-aware code goes live.
- FREEDOM25 / "25 Days of Freedom": concept + copy + 2 hero image options delivered for review; NOT built (awaiting user's format/discount/placement decisions post-conference).
- Lunch Series: pre-order-only, untouched (not part of this batch). Holiday IE ePub: held back pending logo check.
...
- [x] **Dynamic fulfillment messaging** — replaced the stale static `EXPECTED_DELIVERY_DEFAULT = "Expected by Mother's Day (May 10, 2026)"` with reason-aware `fulfillment_note_for(product_id) -> (status, message)` in `payment_routes.py`; statuses: deliverable / online ("Access in your Soul Food Library after you sign in") / physical ("Ships to your address — tracking will be emailed…") / pending ("We'll email your access as soon as it's ready"). `expand_items_for_receipt` + the email template (`email_service.py` `_fulfillment_tag` + status-aware single/bundle rendering) now render these — no more dated placeholder. Render-verified: no "Mother" string in buyer or recipient emails.
- [x] **Public Resend Access Link now accepts the gift recipient** (`routes/redeem_routes.py` `/api/orders/resend-access`): previously validated only `customer_email` → rejected the recipient address shown on the Order Complete page ("Email does not match the order"). Now accepts buyer OR `digital_recipient_email` and sends the correct email type (recipient access / buyer receipt / self). Verified via ephemeral preview txn: recipient→200, buyer→200, stranger→403 (real Resend sends succeeded).
- [x] **Admin Resend (`/api/admin/orders/{order}/resend-email`) is gift-aware** (`routes/admin_routes.py`): resends BOTH the buyer receipt AND the recipient access email; no longer blocks gift resends on missing download links (online-access gifts legitimately have none).
- Note: the production Golden Transaction screenshots (stale Mother's Day text + resend rejection) were from BEFORE these fixes. Fixes live in PREVIEW only — require redeploy to reach kingdom-soul.com. Recommend re-running a gift Golden Transaction post-deploy to confirm recipient receives the email end-to-end.


### P5 — Final UI Polish (June 25, 2026)
- [x] **Responsive presentation pass (June 25, 2026)** — release-readiness check at Desktop 1440 / Tablet 768 & 1024 / Mobile 390 across homepage, Quick Order, Checkout, My Library (iteration_47 → fixes → iteration_48 re-verify). Fixed the user-reported **tablet hero compression**: hero two-column was gated at `lg` (1024px) so iPad-portrait 768 stacked the book cover alone below a long text column — changed to `md:flex-row` + `md:text-left` + `md:mx-0` + `md:justify-start` (SoulFoodApp.js ~571-596) so 768 now renders text-left / cover-right, balanced. Also widened the Quick Order "Small Group Options" card `max-w-xl`→`max-w-2xl` and added `min-h` to July4 tile sub-labels for even tile heights. Verified: NO horizontal scrollbar on any page at any of the 4 breakpoints; mobile still single-column; tiles wrap 2-up cleanly. Remaining items are optional cosmetic — agent marked "acceptable for release."
- [x] **Quick Order "Built for Small Groups" card → community experience (June 25, 2026, per user journey direction)** — swapped `SmallGroupBundleCard` image from the booklet-fan to the community photo (`tile-smallgroup-community.png`, `object-cover`), renamed card title "Small Group Bundles"→**"Small Group Options"**, softened section copy "in one bundle"→"in one place". Tier `label` values LEFT UNCHANGED (they flow into cart item `name` → order history/email/fulfillment); only display surfaces touched. Verified on preview. Journey: Homepage=discover products → Quick Order=choose ministry option → Checkout=purchase → Library=use.
- [x] **Small Group image — community, not product (corrected June 25, 2026)** — per clarified intent, the Small Groups *journey/promo* tiles must show **community/people** (storytelling), while the *product* card keeps the booklet imagery. Generated a new community photo `tile-smallgroup-community.png` (diverse small group around a table with Bibles + fellowship food, Gemini 3.1 Flash Image) and pointed BOTH the homepage Ministry Journey "Community" tile (`SoulFoodApp.js`, `object-cover object-center`) and the July4Banner "Small Group Bundle" featured tile (`July4Banner.js`) to it. The QuickOrder `SmallGroupBundleCard` (the actual product) still uses the booklet-fan `tile-smallgroup.png`. Reinforces the visual distinction: Small Groups→people, Products→books. Verified on preview homepage.
- [x] **Holiday Quick Order tile cropping fixed** — the Featured "Holiday ePub (AE)" tile (`QuickOrder.js`, `data-testid="quick-pick-hol"`) used `h-full w-auto object-contain py-3`, which clipped the cover vertically. Changed to `max-w-[88%] max-h-[92%] object-contain` to match its two sibling Featured tiles (Holiday Table Bundle / Full Table Experience) — bounded, centered, no crop.
- [x] **Reduced repetitive "Bundle" wording** (display-only — NO product IDs, cart-item `name:` values, addToCart logic, or data-testids touched, so order history / email / fulfillment are unaffected): Featured badges `Bundle — Save $4`→`Collection · Save $4` and `Bundle — Save $9`→`Teaching Kit · Save $9`; Game pack badge `Bundle · Save $10`→`Save $10` + feature `Save $10 Bundle`→`Save $10 Together`; Small Group modal button `Add Bundle to Cart`→`Add to Cart`; homepage Starter offer badge `Starter Bundle`→`Starter Collection`. Removes side-by-side word repetition while keeping the core product names ("Holiday Table Bundle", "Small Group Bundles") consistent everywhere.
- Verification: clean webpack compile; July4Banner Small Group tile crop fix visually confirmed on preview. Holiday ePub + journey tiles fixed by mirroring already-correct sibling patterns. Changes are PREVIEW-only pending redeploy.
...
- [x] **Library trust** — `/api/payments/my-purchases` already excludes refunded/cancelled; verified an admin-refunded order leaves the Library immediately while remaining in Order History flagged 'Refunded'. `/api/payments/my-orders` now returns purchased_by/granted_to/owned_by_me/entitlement_status/manual_override + display_status.
- [x] **P4 Purchasing vs Ownership** — Admin order detail (`/api/admin/orders/{n}/detail`) returns an `ownership` block (purchased_by, granted_to, recipient, is_gift, entitlement_status, manual_override, active/revoked link counts). AdminOrders.js expanded view shows an Ownership & Entitlement panel + Mark Refunded / Mark Cancelled / Revoke Access / Reinstate(Paid) buttons + amber override note. OrderHistory.js shows a gift ownership note ("Purchased by you · Granted to X — recipient owns access, not your account"); non-gift completed orders show 'Access in Library'. A gift buyer never appears to own the entitlement.
- [x] **Verified** — backend DB scenario script (refund→links revoked + download blocked + Library excludes + Order History refunded; reinstate restores; manual revoke/grant; ownership fields; revoke scoping confirmed correct, no over-revoke) ALL PASSED; frontend e2e iteration_44.json 100% AFTER fixing a critical crash (missing `Lock` lucide import → resolved to window.Lock 'Illegal constructor' on order expand; tester added the import).
- Notes/backlog: AdminOrders.js 905 lines (refactor); duplicate-key React warning on orders list (LOW, held per console-cleanup freeze); consider an inline "N refunded hidden — switch to All" hint on the visibility filter; seed a gift order in dev to live-test the OrderHistory ownership-note. NEXT PASS: P2 (Dynamic Order/Fulfillment Status + revenue reporting).


### Admin-Managed, Data-Driven Instructor Toolbox + Two Content Lanes (June 25, 2026)
- [x] **Backend `routes/toolbox_routes.py` (NEW)** — dynamic sections + two lanes. Collections: `toolbox_sections` (admin-manageable: key/title/description/icon/order/enabled/is_default/special_view; 6 defaults seeded lazily) and `toolbox_assets` (links `db.files` to a section with `lane` publisher|instructor + `owner_id`). Reuses File Manager storage (Emergent Object Storage) — no new storage code.
  - Admin (`/api/admin/toolbox/...`): sections list/create/update(rename,enable/disable)/reorder/delete(non-default only, cascades asset links); assets list; assign existing `db.files`; upload new (→File Manager + publisher asset); remove asset.
  - Instructor (`/api/instructor/toolbox/...`): list ENABLED sections; list assets (publisher + own instructor-lane); upload own (instructor lane, private); delete own only; authed download (publisher or own).
- [x] **Admin UI `AdminToolbox.js` (NEW)** at sidebar 'Instructor Toolbox' (`/admin/instructor-content`, replaced the old non-functional scaffold). Left: section list with add/rename/enable-disable/reorder/delete + publisher/instructor counts. Right: section assets with Upload New, Assign Existing (File Manager picker modal), remove.
- [x] **Instructor Toolbox `InstructorToolbox.js` rewired data-driven** — fetches admin-managed sections; each renders its built-in special view if any (maps_gallery / question_bank / offline_games preserved) PLUS a 'Resources' (publisher, read-only) + 'My Class Resources' (instructor uploads own, private, deletable) area via `renderSectionAssets()`. Built-in Game Setup + Achievement Awards kept. Disabled sections hidden. Audio stays in My Library (unchanged). Roster unchanged (instructor-managed).
- [x] **Verified — backend curl matrix** (admin upload→instructor sees; instructor upload own→is_mine; download 200; instructor blocked 403 from deleting publisher; section create/rename/disable hides from instructor; assign-existing) AND **frontend e2e iteration_43.json (100%, 15/15 criteria)** across both Admin + Instructor UIs. All test data cleaned up.
- Lanes: **Publisher** (Overflow Harvest/Admin, shared) vs **Instructor** (IE purchaser's private class resources). Admin can now maintain/expand the Toolbox entirely through the UI — no dev per resource.
- Notes/backlog: `InstructorToolbox.js` now 1486 lines (refactor candidate); AssignModal lists max 100 File Manager files (no pagination yet); dead placeholder switch-cases (answer-keys/facilitation/roster/resources) are now unreachable and can be pruned.


### Pass 1 — My Library vs Order History Separation (June 25, 2026)
- [x] **My Library (`GET /api/payments/my-purchases`) rewritten to entitlement-only** — new `_is_active_entitlement_txn()` filter returns ONLY active, usable, owned content. Excludes: refunded / partial_refund / chargeback (`refund_status`), cancelled / revoked / expired / failed (`status`), admin hygiene records (`is_archived` or `tag=='test'`). Gift ownership enforced: a `purchase_type=='gift'` belongs ONLY to the recipient (`digital_recipient_email`) — a gift bought for someone else never shows in the buyer's Library; a gift received shows in the recipient's Library.
- [x] **Order History (`GET /api/payments/my-orders`) — NEW endpoint** — buyer-centric permanent record of EVERY order placed (paid, pending, refunded, partial, cancelled, revoked, expired, gifts sent) with a computed `display_status`, `is_gift`, `digital_recipient_email`, items, total. Excludes only internal admin hygiene (archived/test). Received gifts intentionally live in Library, not the recipient's order history (Amazon/Kindle pattern).
- [x] **Frontend** — new `OrderHistory.js` page mounted at `/order-history` (status badges, gift labels, per-order items + totals, bidirectional nav). `MyLibrary.js` 'My Content' card gets subtitle "Active content you own and can access right now." + header "Order History" button + in-card link. App.js route + import added.
- [x] **Verified — backend scenario matrix (all PASS, synthetic data cleaned up):** refund → leaves Library (10→2) stays in History as 'refunded'; test-tag → hidden from BOTH; gift-out → not in buyer Library but in buyer History (→recipient email); gift-in → in recipient Library (10→11) not in recipient's own History. **Frontend (iteration_41.json, 100%):** Library shows 10 active entitlements; Order History shows 14 orders (11 Pending + 3 Completed); separation proven (pending orders in History but not Library); all testids + bidirectional nav working.
- Note: `OrderLookup.js` (guest order# + email lookup at `/orders/lookup`) left unchanged — it's the guest-facing single-order lookup, complementary to the logged-in Order History.
