...
- [x] **Small Group image centered/uncropped** — the booklet-fan `tile-smallgroup.png` was being center-cropped by `object-cover` in two places: the July4Banner featured tile (`July4Banner.js` — added per-tile `imgClass: 'object-contain p-1.5'` + made the base class use `p.imgClass || 'object-cover'`) and the homepage Ministry Journey "Community" tile (`SoulFoodApp.js` — `object-cover`→`object-contain p-3` on white bg). The full three-booklet fan (Youth · Adult · Adult-Pro) now displays centered without clipping. Verified on preview.
